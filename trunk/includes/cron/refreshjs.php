...
class cron_refreshjs
{
	...
	function docron()
	{
		...
		$cron_time = $DB->queryFirst("SELECT nextrun FROM " . TABLE_PREFIX . "cron WHERE filename = 'refreshjs.php'");
		$this_jss = $DB->query("SELECT * FROM " . TABLE_PREFIX . "javascript WHERE nextrun <= " . $cron_time['nextrun'] . " AND refresh != 0");
		if ($DB->numRows($this_jss))
		{
			while ($js = $DB->fetch($this_jss))
			{
			...
			if ($update_db)
			{
				$DB->queryUnbuffered("UPDATE " . TABLE_PREFIX . "cron SET nextrun='" . $cron_time['nextrun'] . "' WHERE filename = 'refreshjs.php'");
			}
		...
		{
			$new_js = $DB->queryFirst("SELECT * FROM " . TABLE_PREFIX . "javascript WHERE nextrun > " . $cron_time['nextrun'] . " AND refresh != 0 ORDER BY nextrun LIMIT 0, 1");
			if ($new_js)
			{
				$DB->queryUnbuffered("UPDATE " . TABLE_PREFIX . "cron SET nextrun='" . $new_js['nextrun'] . "' WHERE filename = 'refreshjs.php'");
			}
		}
